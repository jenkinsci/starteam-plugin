 ...
 * 
 * Files to remove: Typically folders get removed in starteam and the files get
 * left on disk.
 * 
 * Files to checkout: Files that are out of date, missing, etc.
 * 
 * File Points to remember: When using promotions states/labels file changes may
 * be pushed forward or rolled backwards. Either way, it is difficult (using
 * starteam) to accurately determine the previous build when various different
 * labelling strategies are being used (e.g. promotion states, etc). For this
 * reason we persist a list of the filepoints used upon checkout in the build
 * folder. This is then used to compare current v.s. historic and compute the
 * changelist.
 * 
 * Changes to log: LogEntries for changes. This is information to be written to
 * change log
 */
public class StarTeamChangeSet {
	...
	public void setFilePointsToRemember(
			Collection<StarTeamFilePoint> filePointsToRemember) {
